...
function ensureLedgerPanel(){const history=$('#historyScreen .business-shell'),list=$('#historyList');if(!history||!list||$('#fiscalLedgerPanel'))return;const panel=document.createElement('section');panel.id='fiscalLedgerPanel';panel.className='biz-panel sw76-ledger';panel.innerHTML='<div class="panel-head"><div><h2>Documents fiscaux locaux</h2><p class="muted">Registre immuable des documents fiscaux. Le transport officiel dépend du mode MEV choisi dans Réglages (voir Démarrage pour l’enrôlement).</p></div><button class="btn small" id="refreshFiscalLedger">Actualiser</button></div><div id="fiscalLedgerList" class="sw76-list"><div class="muted">Chargement…</div></div>';list.insertAdjacentElement('afterend',panel);$('#refreshFiscalLedger').onclick=()=>loadLedger().catch(e=>toast(e.message,'error'));void loadLedger()}
