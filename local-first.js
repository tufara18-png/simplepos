...
const DB_NAME='resto360-local-v1';
const DB_VERSION=1;
...
async function cachedGet(url){const snap=await storeGet('snapshots',url);if(!snap)return null;return new Response(JSON.stringify(snap.data),{status:200,headers:{'content-type':'application/json','x-resto360-cache':'1'}})}
function currentAuthHeaders(prefer){const s=JSON.parse(sessionStorage.getItem('resto360-session')||'null');const h={'Content-Type':'application/json',apikey:CFG.supabasePublishableKey||'',Authorization:`Bearer ${s?.access_token||CFG.supabasePublishableKey||''}`};if(prefer)h.Prefer=prefer;return h}
...
window.Resto360Printer={kickDrawer,isAndroidNative:()=>!!androidPrinterPlugin()};
