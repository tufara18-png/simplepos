-- Revenu Québec's registered product declares both "Carte de crédit" and "Carte de débit" as
-- distinct payment modes, but mev-protocol.js hardcoded every card payment to "CRE" -- there
-- was no way for a real débit sale to ever carry the "DEB" modPai the MEV-WEB expects.
--
-- Deliberately a new nullable column, not a change to payments.method's existing check
-- constraint: "card" still means "paid by card" everywhere that already reads it (this
-- function's own p_method = 'card' check for terminal_total, printReceipt, history), so
-- nothing that already works needs to change. card_type only narrows which kind of card, and
-- must stay null for cash/other/left_without_paying where the question doesn't apply.
...
-- Identical to 20260816_000023_finalize_invoice_server_side.sql's finalize_invoice() except for
-- the new p_card_type parameter, its validation, and passing it through to payments.card_type --
-- everything else (balance check, proportional cent allocation, invoice_items linkage,
-- mev_payload, order status) is copied verbatim, not reconstructed from memory.
...
-- Drop the previous 8-argument overload first: Postgres would otherwise keep both signatures
-- around, and a stale PostgREST schema cache could route a call to either one.
