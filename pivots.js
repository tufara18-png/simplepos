...
function seatCount(){return Math.max(guestCount(),maxKnownSeat())}
function seatItems(seat){return [...seatCache.values()].filter(x=>Number(x.seat_number||1)===seat&&x.kitchen_status!=='cancelled'&&Number(x.paid_quantity||0)<Number(x.quantity||1))}
function seatTotal(seat){return seatItems(seat).reduce((s,i)=>s+Number(i.unit_price)*(Number(i.quantity)-Number(i.paid_quantity||0)),0)}
...
function renderSeatBar(){
  ...
  // Positions on an ellipse; the wrapper keeps a stable height so the ticket below
  // does not jump around as seats are added.
  const rx=42,ry=34;
  const dots=seats.map(n=>{
    const a=(-Math.PI/2)+((n-1)/count)*Math.PI*2;
    const x=50+rx*Math.cos(a), y=50+ry*Math.sin(a);
    const items=seatItems(n),total=seatTotal(n);
    const picked=sharing&&shareSeats.has(n);
    const cls=['pivot-seat-dot',n===currentSeat&&!sharing?'active':'',picked?'picked':'',items.length?'has-items':''].filter(Boolean).join(' ');
    return `<button type="button" class="${cls}" style="left:${x}%;top:${y}%" data-seat-dot="${n}" title="Place ${n}"><strong>${n}</strong><span>${items.length?money(total):'—'}</span></button>`;
  }).join('');
  bar.innerHTML=`<div class="pivot-map-head"><span class="pivot-label">${sharing?`Partager « ${esc(seatCache.get(pendingShare)?.name||'')} » : touchez les places`:'Ajouter pour'}</span>${sharing?`<span class="pivot-map-actions"><button type="button" class="pivot-print" data-share-confirm>Partager (${shareSeats.size})</button><button type="button" class="pivot-print" data-share-cancel>Annuler</button></span>`:`<span class="pivot-map-actions"><button type="button" class="pivot-print" data-add-seat>+ Place</button></span>`}</div><div class="pivot-map"><div class="pivot-table"><span>Place ${currentSeat}</span></div>${dots}</div>`;
  bar.querySelectorAll('[data-seat-dot]').forEach(b=>b.onclick=()=>{
    const n=Number(b.dataset.seatDot);
    if(pendingShare){shareSeats.has(n)?shareSeats.delete(n):shareSeats.add(n);renderSeatBar();return}
    currentSeat=n;renderSeatBar();
  });
  bar.querySelector('[data-add-seat]')?.addEventListener('click',()=>$('#guestButton')?.click());
...
.pivot-seat-bar{display:block;padding:4px 0 14px;margin-bottom:2px}.pivot-map-head{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:6px}.pivot-map-actions{display:flex;gap:10px}
.pivot-map{position:relative;height:200px;margin:0 auto;max-width:340px}
.pivot-table{position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);width:38%;height:44%;border-radius:14px;background:#f1edff;border:1px solid #ded5ff;display:grid;place-items:center;color:#4f13ff;font-size:11px;font-weight:850;text-align:center;padding:4px}
.pivot-seat-dot{position:absolute;transform:translate(-50%,-50%);min-width:52px;border:1px solid #e2e2e7;background:#fff;border-radius:12px;padding:6px 7px;display:grid;gap:1px;line-height:1.1;box-shadow:0 1px 2px rgba(17,24,39,.05)}
.pivot-seat-dot strong{font-size:13px}.pivot-seat-dot span{font-size:10px;color:#777b85}
.pivot-seat-dot.has-items{border-color:#c9bfff}
.pivot-seat-dot.active{background:#202126;border-color:#202126;color:#fff}.pivot-seat-dot.active span{color:#c9c9d1}
.pivot-seat-dot.picked{background:#4f13ff;border-color:#4f13ff;color:#fff}.pivot-seat-dot.picked span{color:#e4dcff}
